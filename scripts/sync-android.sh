...
# published web bundle was built against the code it is actually running. See
# scripts/native-hash.mjs. Generated rather than committed: it has to be
# derived from the files as they are at build time, or it would lie.
NATIVE_HASH="$(node "$SCRIPT_DIR/native-hash.mjs")"
if [ -z "$NATIVE_HASH" ]; then
  echo "  ERROR: could not compute the native fingerprint."
  echo "         Without it the app can never apply a background web update."
  exit 1
fi
cat > "$RES_DIR/values/covault_build.xml" <<XML
<?xml version="1.0" encoding="utf-8"?>
<!-- Generated by scripts/sync-android.sh. Do not edit. -->
<resources>
    <string name="covault_native_hash">$NATIVE_HASH</string>
</resources>
XML
echo "  OK: native fingerprint $NATIVE_HASH"
