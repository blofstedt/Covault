...
const NotificationSettings: React.FC<NotificationSettingsProps> = ({ enabled, onToggle }) => {
  ...
  if (!isNative) {
    return (
      <div className="flex items-center justify-between p-4 bg-slate-100 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700/50">
  ...
  return (
    <div className="bg-slate-100 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700/50">
      {/* Toggle row */}
      ...
      {/* Banking app picker - only when enabled and permission granted */}
      {enabled && permissionGranted && (
        <div className="px-4 pb-4 pt-0 space-y-2 border-t border-slate-200 dark:border-slate-700/50">
          ...
          {installedBankApps.length === 0 ? (
            <p className="text-[10px] text-slate-400 text-center py-2">
              No supported banking apps detected.
            </p>
          ) : (
              ...
              {installedBankApps.map(app => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center space-x-2 px-2 py-2 rounded-lg text-left transition-all active:scale-95 border ${
                      selected
                        ? 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-300 dark:border-emerald-700/50'
                        : 'bg-white dark:bg-slate-800/60 border-slate-200 dark:border-slate-700/30'
                    }`}
                  >
