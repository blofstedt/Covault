...
const TransactionItem: React.FC<TransactionItemProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 flex items-center justify-end pr-8 rounded-[2rem] transition-colors duration-200 ${deleteReady ? 'bg-rose-500' : 'bg-rose-400/30 dark:bg-rose-900/40'}`}>
        <div className={`flex items-center space-x-2 transition-all duration-200 ${showDeleteHint ? 'opacity-100 scale-100' : 'opacity-0 scale-75'}`}>
          <svg className={`w-5 h-5 ${deleteReady ? 'text-white' : 'text-rose-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
          </svg>
          <span className={`text-[10px] font-black uppercase tracking-widest ${deleteReady ? 'text-white' : 'text-rose-500'}`}>Delete</span>
        </div>
      </div>

      {/* Edit background (swipe right) */}
      <div className={`absolute inset-0 flex items-center justify-start pl-8 rounded-[2rem] transition-colors duration-200 ${editReady ? 'bg-emerald-500' : 'bg-emerald-400/30 dark:bg-emerald-900/40'}`}>
        <div className={`flex items-center space-x-2 transition-all duration-200 ${showEditHint ? 'opacity-100 scale-100' : 'opacity-0 scale-75'}`}>
          <svg className={`w-5 h-5 ${editReady ? 'text-white' : 'text-emerald-600 dark:text-emerald-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <span className={`text-[10px] font-black uppercase tracking-widest ${editReady ? 'text-white' : 'text-emerald-600 dark:text-emerald-400'}`}>Edit</span>
        </div>
      </div>
