...
const Auth: React.FC<{ onSignIn: any }> = () => {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          // Using href instead of origin to ensure we return to this exact preview page
      ...
      <div className="relative space-y-6 mt-auto flex flex-col items-center pb-8">
        ...
        {isLoggingIn ? (
          ...
          >
            ...
            <span className="text-slate-600 dark:text-slate-100 font-black text-sm uppercase tracking-widest">Connect with Google</span>
          </button>
        )}
